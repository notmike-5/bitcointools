...
def sanitize_segwit(tx_hex: int, debug: bool = False) -> int:
    ...
    for _ in range(input_cnt):
        ...
        if debug:
            print(
                f"Input {_+1} Outpoint: txid={txid[::-1].hex()}, vout={int.from_bytes(vout, 'little')}"
            )
        ...
        script_sig = tx[i : i + script_sig_len]
        i += script_sig_len
        if debug:
        ...
        if debug:
            print(
                f"Input {_+1} Sequence: {sequence.hex()} ({int.from_bytes(sequence, 'little'):08x})"
            )
    ...
    if i != len(tx):
        raise ValueError(
            f"Unexpected data after locktime: {len(tx) - i} bytes remaining"
        )
