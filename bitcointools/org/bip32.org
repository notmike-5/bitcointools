* Imports
#+begin_src python :tangle ../bip32.py :results silent :session pybtc
'''
...
class BIP32PublicNode:
    ...
    def from_xkey(cls, xkey: str) -> BIP32PublicNode:
        ...
        if version in PRV_VERSIONS:
            raise ValueError("xprv key passed to BIP32PublicNode.from_xkey() -- use BIP32Node.from_xkey()")
...
class BIP32Node(BIP32PublicNode):
    ...
    def from_xkey(cls, xkey: str) -> BIP32Node:
        ...
        if version in PUB_VERSIONS:
            raise ValueError("xpub passed to BIP32Node.from_xkey() -- use BIP32PublicNode.from_xkey()")
...
def validate_xkey(xkey: str):
    ...
    if is_private:
        ...
        prefix = key_data[0]
        if prefix == 0x00:
            raise ValueError("xpub version but key prefix is 0x00 (looks like xprv key_data)")
        if prefix not in (0x02, 0x03):
            raise ValueError(f"Invalid pubkey prefix: 0x{prefix:02x} (expected 0x02 or 0x03)")
        point_from_compressed_bytes(key_data)  # validates point is on curve and raises if invalid
...
tv2 = { 'name': "BIP-32, Test Vector 2",
        ...
        'tests': { 1: {'chain': "m",
                   ...
                   5: {'chain': "m/0/2147483647'/1/2147483646'",
                   ...
                   6: {'chain': "m/0/2147483647'/1/2147483646'/2",
...
if __name__ == "__main__":
    ...
    for tv in [tv1, tv2, tv3, tv4]:
        ...
        for _, t in tv['tests'].items():
            ...
            assert node.xpub() == expected_xpub, f"Expected: {expected_xpub}\nGot: {node.xpub()}"
            expected_xprv = t['xprv']
            assert node.xprv() == expected_xprv, f"Expected: {expected_xprv}\nGot: {node.xprv()}"
    ...
    for test_num, t in tv5["tests"].items():
        for key_type in ["xprv", "xpub"]:
            ...
            try:
                ...
                print(f"  [{test_num}] {key_type} UNEXPECTEDLY PASSED  # {t[key_type]['comment']}")
            except ValueError as e:
                print(f"  [{test_num}] {key_type} correctly rejected: {e}  # {t[key_type]['comment']}")
