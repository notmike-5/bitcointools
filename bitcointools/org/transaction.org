...
* OutPoint
The OutPoint class encapsulates the (txid, vout) of some UTXO. It may
prove necessary to also include the amount and/or the scriptPubKey,
but for now it does not.

`txid` can be given as a str or bytes, but internally it is always
bytes. easier to handle endianness and serialization this way.

`vout` is a positive int such that 0 <= vout <= 0xffffffff.

By default we produce a coinbase output (32 * b'\x00', 0xffffffff).
...
* TxIn Class
TxIn is an input into a bitcoin transaction. It contains,
    - prevout: OutPoint, a reference to some previous output
    - scriptSig: str|bytes, this is the un-locking script
    - segwit: bool, flag indicating segwit conformance
    - witness: List[int], witness stack for segwit TXs
    - sequence: int, used to enable/disable RBF or nLocktime

As with the txid in OutPoint, scriptSig can be given as str or bytes,
but it is internally always bytes.

Segwit must be selected. Default is non-segwit.

Witness is a list of hex strings (for now).

Sequence
    - If all TxIn in a transaction have 0xffffffff nLocktime is irrelevant.
    - 0xfffffffe enables nLocktime and disables replace-by-fee (RBF)
    - sequences <= 0xfffffffd opt-into replace-by-fee (RBF)

By default, sequence is set to 0xffffffff.
...
class TxIn:
    ...
    def serialize(self) -> bytes:
        ...
        if self.segwit:
            ...
            b += serialize_varbytes(self.scriptSig) # scriptSig as (size byte) (scriptSig raw bytes)
...
* TxOut Class
TxOut is an output in a bitcoin transaction. It contains,
    - amount: int, amount denominated in satoshis 1/1000000 of a bitcoin
    - scriptPubkey: str|bytes, this is the locking script

As usual, scriptPubkey can be given as str or bytes, but it is
internally always bytes.
...
class TxOut:
    ...
    def serialize(self) -> bytes:
        ...
        b += serialize_varbytes(self.scriptPubkey) # scriptPubkey as (size byte) || (scriptPubkey, raw bytes)
...
* Transaction Class
Transaction container has
    - version: int,
        (1) original bitcoin,
        (2) BIP 68, 112, 113
        (3) Bitcoin-Core Opt-in for Topologically Restricted Until
    Confirmation (TRUC)
    - inputs: List[TxIn], a list of inputs we are spending
    - outputs: List[TxOut], a list of outsputs we are sending to
    - locktime: # TO-DO describe this adequately
#+begin_src python :tangle ../transaction.py :results silent :session pybtc
