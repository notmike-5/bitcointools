...
class OutPoint:
    ...
    def __init__(
        self, txid: str | bytes = b"\x00" * 32, vout: int = 0xFFFFFFFF
    ) -> None:
        ...
        match txid:
            ...
            case str():
                if len(txid) != 64 or not isinstance(int(txid, 16), int):
                    raise ValueError(
                        "txid must be a 32-byte hex string for each OutPoint"
                    )
        ...
        if not isinstance(vout, int) or vout < 0:
            raise ValueError(
                "You must provide a positive integer vout for each OutPoint"
            )
    ...
    def assert_valid(self) -> None:
        ...
        if not 0 <= self.vout <= 0xFFFFFFFF:
            raise ValueError(
                f"vout must be an integer 0 <= vout <= 0xffffffff, and {self.vout} is not"
            )
    ...
    def serialize(self) -> bytes:
        ...
        b += self.vout.to_bytes(
            4, byteorder="little", signed=False
        )  # vout, 8-byte, little-endian

        return b


# TODO: better handling of witness/scriptsig. Segwit shouldn't have a scriptsig. Legacy shouldn't have a witness. Technically a user could provide both, and its not currently handled. Also, TxIn could be created before there is a witness since signatures will be on the sighash/common signature message and this depends on the other input info.
class TxIn:
    ...
    def __init__(
        ...
        # prevout
        if not isinstance(prevout, OutPoint):
            raise ValueError("The previous output must be an OutPoint")
        self.prevout = prevout

        # handle legacy scriptSig / segwit witness (if provided)
        if all(x is not None for x in [scriptSig, witness]):
            raise ValueError("Only one of scriptSig or witness can be given")
        elif scriptSig is not None:
            self.add_scriptSig(scriptSig)
        elif witness is not None:
            self.add_witness(witness)
        else:  # default case
            self.segwit = True
            self.scriptSig = b"\x00"
            self.witness = None
    ...
    def add_scriptSig(self, scriptSig: str | bytes) -> None:
        self.segwit = False
        self.witness = None
        match scriptSig:
            case str():
                if not isinstance(int(scriptSig, 16), int):
                    raise ValueError(
                        "You must provide a scriptSig for each legacy input."
                    )
    ...
    def add_witness(self, witness: list[int]) -> None:
        self.segwit = True
    ...
    def serialize(self) -> bytes:
        ...
        if self.segwit:
            ...
            b += serialize_varbytes(
                self.scriptSig
            )  # scriptSig as (size byte) (scriptSig raw bytes)
        b += self.sequence.to_bytes(
            4, byteorder="little", signed=False
...
class TxOut:
    ...
    def __init__(self, amount: int = None, scriptPubkey: str | bytes = None) -> None:
        # nValue
        if amount is None:
        ...
        match scriptPubkey:
            case str():
                if not isinstance(int(scriptPubkey, 16), int):
                    raise ValueError(
                        "You must provide a valid scriptPubkey for every output"
                    )
    ...
    def serialize(self) -> bytes:
        ...
        b += serialize_varbytes(
            self.scriptPubkey
        )  # scriptPubkey as (size byte) || (scriptPubkey, raw bytes)
...
class Transaction:
    ...
    def __init__(
        ...
        if not (isinstance(version, int) and (0 < version <= 3)):
            raise ValueError(
                "version must be in {0x1 (legacy), 0x2 (SegWit), 0x3 (TRUC)}"
            )
        ...
        if self.segwit:
            ...
            self.flag = (
                b"\x01"  # must be flag >= 0x01. subject to future protocol changes.
            )

        # vin - tx inputs
        if not inputs:
            raise ValueError(
                "You must provide a list of valid Tx Inputs that you wish to spend."
            )
        for i in inputs:
            i.assert_valid()
        self.inputs = inputs
        ...
        if not outputs:
            raise ValueError(
                "You must provide a list of valid Tx Outputs that you wish to send to."
            )
        for o in outputs:
            o.assert_valid()
        ...
        if not (0 <= locktime <= 0xFFFFFFFF):
            raise ValueError(
                "locktime must be a positive integer s.t. 0 <= locktime <= 0xffffffff"
            )
    ...
    def serialize(self, include_witness: bool) -> bytes:
        ...
        if segwit:
            for i in self.inputs:
                if i.segwit:
                    b += i.stack_size  # add stack size
                    for _ in i.witness:
                        b += serialize_varbytes(bytes.fromhex(_))  # add each stack item
...
Sat_Hal_test = {
    "desc": "The first Bitcoin transaction between Satoshi and Hal Finney",
    "version": 1,
    "Vin": [
        {
...
P2SH_test = {
    "desc": "An early P2SH Transaction",
    "version": 1,
    "Vin": [
        {
...
Segwit_v0_test = {
    "desc": "An early Segwit v0 transaction",
    "version": 2,
    "Vin": [
        {
            "txid": "8c655da543f574b6af8b1cdeb15cf3cdee89395e43c574264521e81f6ede533a",
            "vout": 54,
            "witness": [
                "304402201250febbce0a5b333c2d715b869cb960f5abf1702192c7af6e112c6d6030be880220073c55f4814a064bf804d9ed16b57eaaeaafb536c4187e6260ef3fc61ca98a7701",
...
P2TR_scriptpath_and_keypath_test = {
    "desc": "the first transaction with both a P2TR scriptpath and a P2TR keypath input",
    "version": 2,
    "Vin": [
        {
            "segwit": True,
...
p2tr_multi_keypath_input_test = {
    "desc": "early P2TR with multiple P2TR keypath inputs",
    "version": 2,
    "Vin": [
        {
            "segwit": True,
...
p2tr_scriptpath_2_of_2_multisig_spend_test = {
    "desc": "the first p2tr scriptpath 2-of-2 multisig spend",
    "version": 2,
    "Vin": [
        {
            "segwit": True,
...
p2tr_opchecksigadd_test = {
    "desc": "the first use of the new Tapscript opcode OP_CHECKSIGADD",
    "version": 1,
    "Vin": [
        {
            "segwit": True,
...
def run_test(test: dict):
    ...
    outputs = []
    for o in test["Vout"]:
        outputs.append(TxOut(amount=o["amount"], scriptPubkey=o["scriptPubkey"]))

    # locktime
    locktime = test["nLocktime"] if "nLocktime" in test else 0

    tx = Transaction(
        version=test["version"], inputs=inputs, outputs=outputs, locktime=locktime
    )
    ...
    assert (
        tx.get_txid() == test["TXID"]
    ), f"\nTest Failed: {test['desc']}\nExpected: {test['TXID']}\nGot:\t{tx.get_txid()}"
    assert (
        tx.serialize(include_witness=True).hex() == test["HEX"]
    ), f"\nTest Failed: {test['desc']}\nExpected:{test['HEX']}\nGot:\t {tx.serialize(include_witness=True).hex()}"
    assert (
        tx.get_txhash() == test["HASH"]
    ), f"\nTest Failed: {test['desc']}\nExpected:{test['HASH']}\nGot:\t {tx.get_txhash()}"
...
def run_tests():
    ...
    print(
        "\nHidden Message!\t",
        hex_to_ascii(First_P2TR_test["Vout"][0]["scriptPubkey"][4:]),
    )
    ...
    print(
        "\nHidden Message!\t",
        hex_to_ascii(
            p2tr_scriptpath_2_of_2_multisig_spend_test["Vout"][0]["scriptPubkey"][4:]
        ),
    )
