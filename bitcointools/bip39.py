...
class Mnemonic:
    def __init__(self, language: str = "english", wordlist: list[str] | None = None):
        ...
        if wordlist is None:
            d = os.path.join(
                os.path.dirname(__file__), f"test/bip39/wordlist/{language}.txt"
            )
        ...
        # Japanese must be joined by idographic space
    ...
    def to_seed(cls, mnemonic: str, passphrase: str = "") -> bytes:
        ...
        passphrase = "mnemonic" + cls.normalize_string(passphrase)
...
def run_tests():
    # Trezor tests
    with open("bitcointools/test/bip39/trezor_bip39_test_vectors.json", "r") as f:
        vectors = json.load(f)
        for language in vectors.keys():
            mnemonic = Mnemonic(language)
            for v in vectors[language]:
                code = mnemonic.to_mnemonic(bytes.fromhex(v[0]))
                seed = Mnemonic.to_seed(code, passphrase="TREZOR")
                # xprv = Mnemonic.to_hd_master_key(seed)
                assert mnemonic.check(v[1]) == True
                assert code == v[1]
                assert seed.hex() == v[2]
                # assertEqual(v[3], xprv, language)
